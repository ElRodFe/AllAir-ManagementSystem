...
export default function Dashboard() {
  ...
  useEffect(() => {
    // if your protected route already ensures auth, simply read stored user for header display
    const stored = localStorage.getItem("user");
    if (stored) setUser(JSON.parse(stored));
    // always load data (protected route should ensure tokens)
    loadData();
  }, []);
  ...
  const ordersWithCustomerInfo = useMemo(() => {
    return rawData.map(order => {
      ...
      return {
        ...
        customer_phone: cust ? cust.phone_number || cust.phone : "N/A",
    ...
    <>
      ...
      <div className="dashboard">
        ...
        <div className="overview-box flex">
          <StatCard icon_url="assets/total_orders.svg" label="Total Orders" value={stats.total} color="orange" />
          <StatCard icon_url="assets/new_orders.svg" label="New Orders" value={stats.new} color="blue" />
          <StatCard icon_url="assets/completed_orders.svg" label="Completed Orders" value={stats.completed} color="green" />
          <StatCard icon_url="assets/pending_orders.svg" label="Pending Orders" value={stats.pending} color="red" />
        </div>
